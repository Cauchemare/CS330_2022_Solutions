...
def get_dataset(dataset: str, n_train: int, n_val: int = 100):
    if dataset == "cnn":
        n_train = 64
        d = datasets.load_dataset("cnn_dailymail", "3.0.0", split="train")  #Q
        ...
        try:
            d = datasets.load_dataset("csv", data_files=data_files)["train"]  #Q
        ...
        train = defaultdict(lambda: [None] * 5 * n_train)  #[ 'r_b0','r_b1','r_b2','r_b3','r_b4','r_b5' ]
        val = defaultdict(lambda: [None] * 5 * n_val)  #[ '0','1','2','3','4','0',... ]
...
def get_model_and_tokenizer(model: str, Cls, **model_kwargs):
    ...
    if isinstance(m, transformers.GPT2LMHeadModel):
        m.transformer.gradient_checkpointing_enable() #Q

    tok = transformers.AutoTokenizer.from_pretrained(hf_model_name)  #Q

    if tok.pad_token_id is None:   #Q
        if Cls == transformers.AutoModelForCausalLM:#Q
