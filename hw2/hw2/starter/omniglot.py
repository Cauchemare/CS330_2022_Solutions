...
class OmniglotDataset(dataset.Dataset):
    ...
    def __init__(self, num_support, num_query):
        ...
        self._character_folders = glob.glob(
            os.path.join(self._BASE_PATH, '*/*/'))  #R
        assert len(self._character_folders) == (
            NUM_TRAIN_CLASSES + NUM_VAL_CLASSES + NUM_TEST_CLASSES
        )

        # shuffle characters
        np.random.default_rng(0).shuffle(self._character_folders)  #R
    ...
    def __getitem__(self, class_idxs):
        ...
        for label, class_idx in enumerate(class_idxs):
            ...
            all_file_paths = glob.glob(
                os.path.join(self._character_folders[class_idx], '*.png')  #R
            )
        ...
        images_support = torch.stack(images_support)  # shape (N*S, C, H, W)   # Q
...
def get_omniglot_dataloader(
    ...
    return dataloader.DataLoader(
        ...
        num_workers=8,
        collate_fn=identity, #Q
        pin_memory=torch.cuda.is_available(),  #Q
        drop_last=True
    )
