...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        Args:
            task_batch (tuple[Tensor, Tensor, Tensor, Tensor]):
                ...
                for each task: return 4 tensor as follow(reference to  OmniglotDataset.__getitem__:
        ...
        for task in task_batch:
            ...
            # 1.calculaute prototype with  f(x),only for support_dataset
            N_SUPPORT = int(self._log_dir.split('.')[3].split(':')[1])
             
            NS= images_support.shape[0]  #n*s
            NQ= images_query.shape[0]  # n*q
            
            assert NS % N_SUPPORT ==0
            N_CLASS=  NS //N_SUPPORT
            prototype_list =[self._network( images_support[idx*N_SUPPORT:(idx+1)*N_SUPPORT,... ]).mean(dim=0)  for  idx in  range(N_CLASS)] 
            prototypes_embedding=  torch.stack( prototype_list )    # (NUM_CLASS,EMBEDDING_DIM) 
            
            # 2.calculate support_accuracy,query_accuracy
            images_support_embedding = self._network(  images_support)    #(S*N,EMBEDDING_DIM)
            images_query_embedding  = self._network( images_query)  #(Q*N,EMBEDDING_DIM ) 
            distances_support = torch.cdist( images_support_embedding ,prototypes_embedding ,p=2  )   # (NQ,N_CLASS)              
            accuracy_support = util.score(  -1*distances_support, labels_support )      
            
              
            distances_query = torch.cdist( images_query_embedding,prototypes_embedding ,p=2   )   # (NQ,N_CLASS)              
            accuracy_query = util.score(  -1*distances_query, labels_query )            
                   
            loss= F.cross_entropy( -1*distances_query,labels_query)
    ...
    def train(self, dataloader_train, dataloader_val, writer):
        ...
        for i_step, task_batch in enumerate(
                dataloader_train,
                start=self._start_train_step #Q
        ):
    ...
    def test(self, dataloader_test):
        ...
        mean_95_confidence_interval = 1.96 * std / np.sqrt(NUM_TEST_TASKS) # Q 1.96
        print(
...
def main(args):
    ...
    if log_dir is None:
        log_dir = f'./logs/protonet/omniglot.way:{args.num_way}.support:{args.num_support}.query:{args.num_query}.lr:{args.learning_rate}.batch_size:{args.batch_size}'  # pylint: disable=line-too-long Q
