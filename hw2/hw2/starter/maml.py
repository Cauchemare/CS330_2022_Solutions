...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for idx in  range(self._num_inner_steps):
            predict= self._forward(images, parameters)
            loss= F.cross_entropy(predict,labels ) 
            accuracy = utils.score( predict,labels)
            accuracies.append(accuracy)
            
            gradients=  autograd.grad(loss, list(paramters.values()), create_graph =train )
            
            for idx,parameter in enumerate(parameters.keys())  :
                gradient =  gradients[idx]
                parameter -=  self._inner_lrs[k]*gradient
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            parameters,accuracy_support= self._innter_loop( images_support,labels_support,train )
            accuracies_support_batch.append(accuracy_support)


            predict_query= self._forward(images_query , parameters)
            loss_query  = F.cross_entropy(predict_query,labels_query ) 
            accuracy_query = util.score(  predict_query, labels_query  )
