...
class MultiTaskNet(nn.Module):
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        for layer_idx  in  range(len( layer_sizes) -1) :
            mlp_layers.append( nn.Linear(layer_sizes[layer_idx] ,  layer_sizes[layer_idx+1] ))
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions=  u @ q.T +b #  
        predictions=  predictions[torch.eye(len(user_ids))>0 ]
        # Regression head  
        layer_inputs= torch.concat([   u, q,u*q ] ,dim =1)
        score= self.mlp_layers( layer_inputs).ravel()    
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Matrix Factorization Head
        predictions=  u_fact @ q_fact.T+b 
        predictions=  predictions[torch.eye(len(user_ids)) >0 ]
        # Regression head 
        layer_inputs= torch.concat([   u_reg, q_reg,u_reg*q_reg ] ,dim =1)   #B*96
        score= self.mlp_layers( layer_inputs).ravel()              
