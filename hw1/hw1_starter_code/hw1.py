...
class MANN(nn.Module):
    def __init__(self, num_classes, samples_per_class, hidden_dim):
        ...
        self.samples_per_class = samples_per_class  #K+1
    ...
    def forward(self, input_images, input_labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        input_labels= input_labels.detach().clone()  #Q
        B,K,N,N= input_labels.shape
        input_labels[:,-1,:,:]= 0 
        x=torch.concat([input_images,input_labels],dim=-1).reshape( B,K*N,-1)
        x= x.float()  #Q
        for model in [self.layer1,self.layer2 ]:
            x,_ =  model(x)      
        return x.reshape(B,K,N,N)  
    ...
    def loss_function(self, preds, labels):
        ...
        loss = F.cross_entropy( preds[:,-1,:,:],labels.float() [:,-1,:,:] )  #Q
        return  loss
...
def train_step(images, labels, model, optim, eval=False):
    ...
    if not eval:
        ...
        loss.backward()
        optim.step()
    return predictions.detach(), loss.detach()  #Q
